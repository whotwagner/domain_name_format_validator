...
class DNFVHostnamesTest < Minitest::Test
  def setup
    @debug = false
    @mode = DomainNameFormatValidator::Mode::HOSTNAME
  end

 def is_valid_domain?(domain)
   puts domain if @debug
   DomainNameFormatValidator.valid?(domain, @mode)
 end
  ...
  def test_if_label_bigger_as_63_chars_allowed
    ...
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:max_label_size]
  ...
  def test_if_label_is_zero
    ...
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:illegal_chars]
  end

  def test_nil_is_not_allowed
    domain = nil
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:zero_size]
  end
  ...
  def test_all_numeric_tld_is_not_allowed
    ...
    domain = "example.123"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:top_numerical]
  end

  def test_tld_is_too_short
    domain = "example.a"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:bogus_tld]
  end
  ...
  def test_fail_when_max_number_of_levels_is_exceeded
    domain = "a."*127 + "com"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:max_level_size]
  end
  ...
  def test_if_label_does_not_start_or_end_with_hiphen
    ...
    domain = "-example.com"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:label_dash_begin]
    # hyphens at the end are not allowed
    domain = "example-.com"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:label_dash_end]
  ...
  def test_if_entire_hostname_is_not_bigger_than_253
    ...
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:max_domain_size]
  end

  def test_if_underscore_allowed
    refute is_valid_domain?('_jabber.example.com')
    refute is_valid_domain?('_jabber.example_.com')
    refute is_valid_domain?('_jabber_._exa-mple_.com')
    domain = "_jabber_.example_._com"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:top_illegal_chars]
    domain = "_jabber_.example_.com_"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:top_illegal_chars]
    refute is_valid_domain?('_jabber_.example_.co-m')
  end
  ...
  def test_domain_must_be_a_string
    domain = ["hello.com"]
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:not_a_string]
  ...
  def test_only_one_level
    domain = "hello"
    refute is_valid_domain?(domain)
    assert_includes DomainNameFormatValidator.errors(domain, @mode), DomainNameFormatValidator::ERRS[:min_level_size]
  end
